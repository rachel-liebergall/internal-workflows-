---
name: hubspot-deal-signal-scanner-baxter
description: Scan HubSpot activity for deal signals and post a summary to #sales-tracking (sent as Baxter).
...
## OUTPUT
Send the message to Slack channel ID C0BAJHSBHHB (#sales-tracking) via the Slack Web API using Baxter's bot token (see SLACK section below).
...
**To post to a channel:** POST to `https://slack.com/api/chat.postMessage` with body `{"channel": "C0BAJHSBHHB", "text": "YOUR MESSAGE", "mrkdwn": true}` and header `Authorization: Bearer xoxb-9016426745090-11552804698144-hyDHp1jttz9c52NSWjq2yov5`.
