...
**To send a DM:** POST to `https://slack.com/api/conversations.open` with body `{"users": "SLACK_USER_ID"}` and header `Authorization: Bearer xoxb-9016426745090-11552804698144-tj4OD37admTTZyBYFlkHlYgp` to get the channel ID. Then POST to `https://slack.com/api/chat.postMessage` with `{"channel": "DM_CHANNEL_ID", "text": "YOUR MESSAGE", "mrkdwn": true}`.
