---
name: meeting-prep-guide-creator-baxter
description: Detect upcoming external meetings, create Notion prep guides, and send Slack notifications (sent as Baxter).
...
Fetch the current UTC time. This is used to determine which notification windows each meeting falls into.

For displaying meeting times in Slack messages and prep guides, always parse the local time directly from the event's ISO 8601 datetime string (e.g. "2026-06-16T10:00:00-05:00" → "10:00 AM CDT"). Do not convert to a fixed timezone. Translate the UTC offset to a common abbreviation where possible (-04:00 → EDT, -05:00 → CDT or EST, -06:00 → CST). This ensures each recipient sees the correct time for wherever they currently are.
...
- Meeting title
- Start datetime (store both UTC and the local time parsed from the event's ISO 8601 string)
...
- Owner Notes: set to "48h_sent" immediately after creation
...
- Meeting date: [date and local time with timezone abbreviation, e.g. 10:00 AM CDT]
- NTN attendees: [names]
- Meeting title: [title]
...
Send to each relevant NTN team member (Rachel always; Jess if on meeting; Jason if on meeting).

### Pre-meeting (premeet list)
Always individual:
"⏰ *Starting in ~1 hour: [meeting title]*

*Time:* [local time with timezone abbreviation, e.g. 10:00 AM CDT]
*External:* [names]
